<template>
  <AppLayout>
    <v-container class="py-4 min-h-screen">
      <v-card class="mb-4" outlined>
        ...
        <v-card-text>
          <v-row dense>
            ...
            <v-col cols="12" md="6" v-if="dateType === 'exact'">
              <v-date-input v-model="storyDate" label="Vyberte datum" min-width="290px"></v-date-input>
            </v-col>

            <v-col cols="12" md="6" v-else-if="dateType === 'year'">
              <v-text-field
                v-model="storyYear"
                label="Rok příběhu"
                type="number"
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" @click="publishStory">Publikovat příběh</v-btn>
        </v-card-actions>
      </v-card>

      <div class="h-full min-h-screen">
        <QuillEditor theme="snow" toolbar="full" class="h-full min-h-screen" />
      </div>
    </v-container>
  </AppLayout>
</template>
